  ...
  setup(props) {
    ...
    const renderAxis = (data, options) => {
      ...
      const yAxisStyle = {
        ...
        textAnchor: 'start',
        filter: `drop-shadow(1px 1px ${yAxisShadowColor})`,
        strokeWidth: 1
      };
      // create a horizontal axis path
      axisChildren.push(lineNode(originData[0], originData[0] + width, originData[1], originData[1], xAxisStyle));

      // create vertical axis path
      axisChildren.push(lineNode(originData[0], originData[0], originData[1], originData[1] - yLen, xAxisStyle));
    ...
    const renderDepth = (data, options) => {
      ...
      const defNode = h('defs', [
          h('linearGradient', {
            ...
            h('stop', { offset: '0%', 'stop-color': buyLinearGradientArray[0] }),
            h('stop', { offset: '30%', 'stop-color': buyLinearGradientArray[1] }),
            h('stop', { offset: '50%', 'stop-color': buyLinearGradientArray[2] }),
            h('stop', { offset: '80%', 'stop-color': buyLinearGradientArray[3] }),
            h('stop', { offset: '100%', 'stop-color': buyLinearGradientArray[4] }),
          ]),
          h('linearGradient', {
            ...
            h('stop', { offset: '0%', 'stop-color': sellLinearGradientArray[0] }),
            h('stop', { offset: '30%', 'stop-color': sellLinearGradientArray[1] }),
            h('stop', { offset: '50%', 'stop-color': sellLinearGradientArray[2] }),
            h('stop', { offset: '80%', 'stop-color': sellLinearGradientArray[3] }),
            h('stop', { offset: '100%', 'stop-color': sellLinearGradientArray[4] }),
          ]),
      ]);
      ...
      let buyPathD = buyPoint.reduce((s, v) => {
        s += s ? ` L ${v.x} ${v.y}` : `M ${v.x} ${v.y}`;
        return s;
      }, '');
      depthChildren.push(h('path', {
        d: buyPathD,
        style: buyStyle
      }));
      ...
      // connect the corresponding points
      let sellPathD = sellPoint.reduce((s, v) => {
        s += s ? ` L ${v.x} ${v.y}` : `M ${v.x} ${v.y}`;
        return s;
      }, '');
      depthChildren.push(h('path', {
      ...
      for (const key of pointMap.keys()) {
        ...
        if (offsetX < x) {
          ...
          if (tipType === "mouse") {
            ...
            const marginTop = 25;
            const lineHeight = 20;
            tipChildren.push(textNode(left + marginLeft, top + marginTop, tipsPriceText, {
              fill: side === "buy" ? tipBuyTextColor : tipSellTextColor,
              textAlign: "left",
              showBluer: 0,
              fontSize: `${tipContextFontSize}px`,
            }))
            tipChildren.push(textNode(left + marginLeft, top + marginTop + lineHeight, tipsAmountText, {
              fill: side === "buy" ? tipBuyTextColor : tipSellTextColor,
              textAlign: "left",
              showBluer: 0,
              fontSize: `${tipContextFontSize}px`,
            ...
            const xAxisToolTipD = `M ${x} ${height} L ${x - 5} ${ height + 5} L ${x - tipsPriceWidth / 2} ${ height + 5} L ${x - tipsPriceWidth / 2} ${height + 5 + lineHeight}
             L ${x + tipsPriceWidth / 2} ${height + 5 + lineHeight} L ${x + tipsPriceWidth / 2} ${height + 5} L ${x + 5} ${height + 5} L ${x} ${height}`;
            tipChildren.push(h('path', {
              d: xAxisToolTipD,
              style: toopTipStyle
            }));
            ...
            const yAxisToolTipD = `M ${1} ${y} L ${8} ${y - 10} L ${8 + tipsAmountWidth} ${y - 10} L ${8 + tipsAmountWidth} ${y + 10} L ${8} ${y + 10} L ${1} ${y}`;
            tipChildren.push(h('path', {
              d: yAxisToolTipD,
              style: toopTipStyle
            }));
    ...
    watchEffect(() => {
      options = Object.assign(props.theme === "night" ? defaultNightOptions : defaultDayOptions, props.customizeOptions);
      pointMap = new Map();
    ...
    return () => h('div',
        ...
        [
            ...
            }, [gridNode.value, axisNode.value, depthNode.value, tipNode.value])
        ]);
  }
}
</script>
